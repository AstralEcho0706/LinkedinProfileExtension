<template>
  <div class="app-container mx-auto">
    <div class="info-details">
      ...
      <div class="user-avatar d-flex justify-content-start align-items-center">
        <div class="avatar">
          ...
          <img class="img-fluid" :src="profilePhoto" style="height: 54px; width: 54px; border-radius: 10px"
            v-if="profilePhoto" />
        </div>
    ...
    <div class="email-address-info">
      <!-- Email List -->
      <div class="email-list d-flex justify-content-between align-items-center" v-if="!emailAddress.length">
        <div class="email-checkbox">
          <div class="form-checkmark d-flex align-items-center">
            <input class="form-check-input" type="checkbox" v-model="emailChecked"
              :id="isSales ? 'email-checkmark-sales' : 'email-checkmark1'" />
            <label class="form-check-label" :for="isSales ? 'email-checkmark-sales' : 'email-checkmark1'">Email address
              <br />
            </label>
          </div>
        </div>
      </div>
      <div class="email-list d-flex justify-content-between align-items-center" v-if="!phoneNumbers.length">
        <div class="email-checkbox">
          <div class="form-checkmark d-flex align-items-center">
            <input class="form-check-input" type="checkbox" v-model="mobileChecked"
              :id="isSales ? 'phone-checkmark-sales' : 'phone-checkmark1'" />
            <label class="form-check-label" :for="isSales ? 'phone-checkmark-sales' : 'phone-checkmark1'">Phone</label>
          </div>
        </div>
      </div>

      <!-- Reveal Button start-->
      <div class="reveal-btn-info text-center" v-if="!emailAddress.length || !phoneNumbers.length">
      ...
      <div class="email-list" v-if="phoneNumbers.length" style="margin-top: 20px">
    ...
    <div class="kompass-ai-info" v-if="companyName">
      ...
      <div class="kompass-company-info d-flex align-items-center">
        ...
        <div class="social-links d-flex align-items-center">
          <span icon="pi pi-check" class="rounded" text raised rounded aria-label="Filter">
      ...
      <div class="box-grid-layout">
        <div class="d-flex align-content-between flex-wrap justify-content-between">
          <div class="box">
      ...
      <div class="experience-company">
        ...
        <div class="kompass-company-info d-flex align-items-flex-start" v-for="item in experiences"
          :key="item.entityUrn">
          <div class="company-logo">
            <img class="img-fluid" :src="item.logoUrl" style="width: 40px" v-if="item.logoUrl" />
            <img class="img-fluid" :src="companyImg" style="width: 40px" v-else />
          </div>
          <div class="kompass-name-info">
            <div class="kompass-title-info" style="width: 75%;">
              <h3>{{ item.title }}</h3>
            </div>
            <span>{{ item.subtitle }}</span>
            <div class="kompass-experience-info" v-if="item.subSkills">
              <div class="kompass-experience-info" v-for="it in item.subSkills" :key="it.title">
                <h3>{{ it.title }}</h3>
          ...
          <div class="social-links">
            ...
            <span icon="pi pi-check" class="rounded" text raised rounded aria-label="Filter">
              <img class="img-fluid" :src="bookmark_border1" @click="saveCompany(item)" />
            </span>
          </div>
        </div>
      </div>
      <div class="skill-tags">
        ...
        <strong id="moreBtn" v-if="skills.length > 5" @click="isExpand = !isExpand">{{ isExpand ? 'Less' : 'Expand'
          }}</strong>
      </div>
    </div>
  </div>
...
const emit = defineEmits(['close, setLoading, loadContacts, saveCompany']);
...
onMounted(() => {
  ...
  isSales.value = window.location.href.includes('linkedin.com/sales/');
});
